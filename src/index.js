...
const server = new Server({ name: "vibeguard", version: "0.1.4" }, { capabilities: { tools: {}, resources: {} } });
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: [
  ...
  { name: "hard_db_audit", description: "Runs a strict database integrity audit prompt. Use only when the user explicitly says: hard db audit.", inputSchema: { type: "object", properties: { projectPath: { type: "string" }, projectContext: { type: "string" }, focus: { type: "string" }, files: { type: "string" }, diff: { type: "string" }, findings: { type: "array" } }, required: ["projectPath"] } }
] }));
